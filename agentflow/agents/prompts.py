...
SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "log-analyst": (
        ...
        "5. **`services` 必须填**：把 `query_logs` 返回的 `by_service` 里**计数 > 0** 的"
        "服务名原样抄进去（只列你确实查到了错误日志的那些）。\n"
        "   这不是可选项——`code-locator` 在 trace 给不出 `failing_service` 时"
        "（工单没带 trace_id）**只能靠它**定位要改哪个仓库；留空等于让整条诊断链在"
        "「不知道该改哪个服务」处停住。\n"
        "   查不到任何错误日志时给空数组，**不要编服务名**。\n"
        f"6. {_JSON_RULE}\n"
    ...
    "code-locator": (
        ...
        "4. ⚠️ **`target_service` 为空/缺失时，按下面的顺序定目标，不要凭空猜**：\n"
        "   ① 入参 `target_service` 非空 → 直接用它（trace 判定的故障服务，**最强**），"
        "并把 `target_source` 填 `\"trace\"`；\n"
        "   ② 否则看入参 `log_services`（上游 log-analyst 给的「窗口内实际报过错的服务」）：\n"
        "      非空 → 取其中**与工单描述最对得上的那一个**用它，"
        "并把 `target_source` 填 `\"logs\"`（弱证据，可能含症状服务）；\n"
        "   来源用 `target_source` 这个**字段**表达，**不要在 summary 里展开解释**"
        "（summary 越长越容易被截断，实测因此挂过整条 run）；\n"
        "   ③ 两个都空 → 直接输出 `found: false`，在 summary 里说明缺什么、"
        "`missing` 数组里逐条列出（如 `\"trace 的 failing_service（工单未提供 trace_id）\"`），"
        "然后**结束**。\n"
        "   判据：**能用的是「证据里出现过的服务」，不是「你觉得像的服务」**——\n"
        "   凭印象猜服务既定位不准（猜错仓库会让下游改错代码），又会让迭代耗尽。\n"
...
REMEDIATION_PLANNING_PROMPT = (
    ...
    "9. **控制篇幅**（写长了会在输出上限处被截断 → 整段 JSON 作废 → 节点失败、整条 run 中止；\n"
    "   而且本次输出预算与你的思考过程**共用**，留给正文的余量比看上去小）：\n"
    "   summary ≤ 80 字；每条 step 的 change ≤ 150 字，只讲「改哪个文件哪段逻辑、怎么改」，**不要贴代码**；\n"
    "   suggested_diff 可选——给它就**只给最小示意（≤ 15 行）**，绝不整段粘文件；\n"
    "   steps ≤ 6 条；risks / assumptions / success_criteria 各 ≤ 3 条、每条 ≤ 80 字；\n"
    "   expected_effect / verification / rollback 各 ≤ 50 字。要说的多就挑最要紧的说，别靠长度显得周全。\n"
    "\n"
    "10. 交付前自检，任一不过则先改写再输出：\n"
    ...
    "   - **篇幅在规则 9 的限额内**（超了就删，别指望「写全一点更好」——超限等于整份作废）。\n"
    "\n"
