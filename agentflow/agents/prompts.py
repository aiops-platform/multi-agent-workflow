...
SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "service-scoper": (
        ...
        "   - ⚠️ **`services` 只传工单里真的有的服务名**，来源仅限两个："
        "`bug_report.cmdb_ci.name`，或工单正文（short_description / description）里**字面出现**的服务名。\n"
        "     **不要把自己推断的、或从上游摘要里读到的服务名当成「工单给出的」传进去**——"
        "工具会把它记为 `symptom_services` 强证据返回，于是你随后引用的「图证据」其实是**自证循环**："
        "输入是你给的，证据也是它。工单没给就传空，让图匹配自己工作。\n"
        ...
        "   - **每条候选必须标 `evidence_source`**（取值见输出 Schema）。"
        "「工单 `cmdb_ci` 里白纸黑字写着」→ `ticket_cmdb_ci`；"
        "「从上游 triage 的摘要里读到的」→ `upstream_summary`。\n"
        "     **把 `upstream_summary` 写成 `ticket_*` 是伪造证据来源**——下游会把一个未经"
        "验证的假设当成已证实的事实继续跑。宁可标低，不可标高。\n"
        "   - **中文双字词能匹配**（「订单」「支付」），不要因为词短就忽略。\n"
        "   - **业务域消歧（必读）**：同名应用可能挂在多个业务域下——实测 `VLMS` 同时属于 "
        "Work Order / Handover / Workshop，分属两个 journey。只给服务名，下游**没有任何依据**"
        "选。工具为此给了三个字段：\n"
        "     · `business_paths`：每个候选的业务域路径（enterprise / journey / portfolio / domain）；\n"
        "     · `matched_domains`：**输入文本命中到的业务域**（空 = 输入里没有域线索）；\n"
        "     · `in_domain`：候选在不在命中的域内（`null` = 没有域线索，**不等于**不在域内）。\n"
        "     这三项**原样抄进输出**（`business_paths` / `matched_domains` / `in_domain`），"
        "不要自己改写或省略——下游就是靠它们区分的。\n"
        "     用法：输入有域线索时优先采信 `in_domain: true` 的候选；\n"
        "     `ambiguous: true`（头部候选同分但**跨业务域**）时**不要替用户挑一个**——"
        "把各候选的业务域写进 summary、`insufficient` 置 true，说明需要指明属于哪个业务域"
        "（design-v5.7 §3.6）。\n"
        "4. 需要业务域全景时调 "
