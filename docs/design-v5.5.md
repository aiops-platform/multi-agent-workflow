...
> ⚠️ **本文档已被 `docs/design-v5.6.md` 取代（2026-09-11）**。v5.6 把本文档（数据面）
> 与 design-v5.4.md（编排层）合并为单一文档，本文档内容**未改动**，仅重编号——本文档
> §2–§8 分别对应 v5.6 §3.1–§3.7 与 §3.6，对照表见 v5.6 §1.4。**请以 v5.6 为准**；
> 本稿保留仅供追溯。
...
| v5.5.2 | 2026-09-11 | **CMDB 并入数据面**：新增 `get_service_topology`（N 跳拓扑，方向相对起点）与 `locate_repo`；CMDB 从 agentflow 进程内的 mock 迁到 server 侧——**租户隔离由部署承载**，agent 进程不再持有服务目录（TODO 中登记的硬编码个人路径与接口无 tenant 两问题一并消除；该条目已移入 TODO §11 留痕） |
