...
> ⚠️ **本文档已被 `docs/design-v5.8.md` 取代（2026-09-21）**。v5.6 先把本文档（数据面）
> 与 design-v5.4.md（编排层）合并，v5.7 在其上增量（CMDB 业务域分层 + 诊断链），
> v5.8 再把三者合一。本文档内容**未改动**，仅重编号——本稿 §2–§8 对应 v5.8 §3.1–§3.8，
> 两轮对照表见 v5.8 §1.4。**请以 v5.8 为准**；本稿保留仅供追溯。
...
| v5.5.2 | 2026-09-11 | **CMDB 并入数据面**：新增 `get_service_topology`（N 跳拓扑，方向相对起点）与 `locate_repo`；CMDB 从 agentflow 进程内的 mock 迁到 server 侧——**租户隔离由部署承载**，agent 进程不再持有服务目录（TODO 中登记的硬编码个人路径与接口无 tenant 两问题一并消除；该条目已移入 TODO §12 留痕） |
