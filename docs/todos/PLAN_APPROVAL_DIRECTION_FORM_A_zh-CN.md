...
> ⚠️ **2026-09-18 变更**：`remediation-planning-analyst` 的 prompt 规则 5 已从「存在互斥修复路径
> **必须**列 decisions[]」改为「**默认只给一个方案**，仅当根因依赖未决前提时才并列互斥备选」，
> 示例 JSON 也改成 `"decisions": []`（见 `agentflow/agents/prompts.py` 该 agent 顶部注释）。
> 原因：原规则 + 示例诱导模型每轮都造出多个决策点（实测 2 决策 × 3 选项 = 6 个「方案」，而 steps
> 只有一份被 6 个选项共用，页面上是一堆正文相同的方案）。
> **本文档描述的 Form A 交互（选方向 → 带方向驳回重写）仍依赖引擎 `on_reject`，尚未实现，
> 未落地部分不受上述改动影响。** 但请注意「decisions[] 每轮都有」这个前提已不再成立——
> 现在它是**少数情况**才出现的备选载体。
>
